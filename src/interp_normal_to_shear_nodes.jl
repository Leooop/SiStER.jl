...
# G.Ito 8/2016
#-------------------------------------------------------------------------
function interp_normal_to_shear_nodes(varN,dx,dy)

    varS=zeros(eltype(varN),size(varN))
    Ny,Nx=size(varS)
    i1, j1 = 2:Ny-1, 2:Nx-1
    i2, j2 = 3:Ny, 3:Nx
    dydx=copy(varS)
    ...
    # corners
    #--------------------------------------------------------------------------
    varS[1,1], varS[1,Nx] = varN[2,2], varN[2,Nx]
    varS[Ny,1], varS[Ny,Nx] = varN[Ny,2], varN[Ny,Nx]
