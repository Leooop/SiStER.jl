...
if (PARAMS.YNPlas == 1)
    dp = p .- pold
    ps = ps_old .+ interp_normal_to_shear_nodes(dp,dx,dy)  #<<<< Interpolation done here
    ps[ps.<0.0] .= 0.0
    pnn = p
    pnn[p.<0.0] .= 0.0
...
if PARAMS.YNElast==1
    ...
    if (t==1) & (pit==1)
        ...
        sII_n = zeros(Float64,size(X))
        sII_s = zeros(Float64,size(X))
    else
        # need sIIOLD_s to initialize bisection (done on shear nodes)
        sIIOLD_s = sqrt.(sxxOLD_s.^2 .+ sxyOLD.^2)
        include("viscosity_stress_bisection.jl") # spits out sII_s consistent with viscosity
        sII_s = copy(sII)
        sII_n = interp_shear_to_normal_nodes(sII_s)
    end
