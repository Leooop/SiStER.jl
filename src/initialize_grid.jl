...
function initialize_grid(xsize,ysize,GRID)
  ...
  # 2D Grid
  #[X,Y] = meshgrid(x,y); matlab version
  X = repeat(x',outer = size(y))
  Y = repeat(y,outer = size(x'))
