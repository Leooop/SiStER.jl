...
dep_s = zeros(Float64, size(epsII_s))
dep_s[s_nodes_yield] = dt_m.*max.(epsII_s[s_nodes_yield].-
(yield_s[s_nodes_yield].-sqrt.(sxxOLD_s[s_nodes_yield].^2+sxyOLD[s_nodes_yield].^2))./(2.0 .*Gs[s_nodes_yield].*dt_m),minimum(epsII_s)*1e-6)

depm = interp_shear_nodes_to_markers(reshape(dep_s,Ny,Nx),x,y,xm,ym,icn,jcn)
