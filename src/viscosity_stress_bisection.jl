...
sIILOW = sIIOLD_s
sIIHIGH = sIIOLD_s
visco0 = get_ductile_rheology_on_nodes_from_stresses(MAT,PARAMS,Ts,sIIOLD_s,phase_s)
if PARAMS.YNElast==1
...
viscoNEW = get_ductile_rheology_on_nodes_from_stresses(MAT,PARAMS,Ts,sIINEW,phase_s)
if PARAMS.YNElast == 1
...
for i = 1:Nbisec
    ...
    sIIPAST = sII
    visco = get_ductile_rheology_on_nodes_from_stresses(MAT,PARAMS,Ts,sII,phase_s)
    if PARAMS.YNElast == 1
