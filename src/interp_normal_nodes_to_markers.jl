...
function interp_normal_nodes_to_markers(varN,xc,yc,xm,ym,icn,jcn)
    ...
    IND = sub2ind.(Ref(LinearIndices(size(varN))), ic, jc)
    VARnodes = [varN[IND] varN[IND.+m] varN[IND .+ 1 .+ m] varN[IND.+1]]

    return interp_grid_to_marker_vector(xNnodes,yNnodes,VARnodes,xm,ym)
