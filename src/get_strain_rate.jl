...
function get_strain_rate(vx,vy,dx,dy,BC)

    Ny, Nx = size(vx)

    EXX=zeros(Float64, size(vx))
    EXY=zeros(Float64, size(vy))
        ...
        for i=1:Ny
            ...
            elseif i==Ny && j>=2 && j<=Nx-1 # bottom
                ...
                elseif BC.bot[1]==0 # no rollers
                    ...
                    dvxdy=2.0*(BC.bot[4]-vx[i-1,j])/dy[i-1]  #G.Ito needed for tangential velocity on base
                end
                EXY[i,j]= 0.5*(dvxdy + 2*(vy[i,j]-vy[i,j-1])/(dx[j-1]+dx[j]))
            ...
            elseif i==1 && j==Nx
                ...
                if BC.top[2]==0 # vy imposed on top (Dirichlet)
                    dvydx=0.0
                elseif BC.top[2]==3 # open top so set to be the same as adjacent node
                     dvydx=2.0*(vy[i,j-1]-vy[i,j-2])./(dx[j-1]+dx[j-2])
                end
                EXY[i,j]=(dvxdy + dvydx)/2
            ...
            elseif i==Ny && j==Nx
                if BC.bot[1]==1 # rollers
                    dvxdy=0.0
                elseif BC.bot[1]==0 # no rollers
                    dvxdy=-2.0*vx[i-1,j]/dy[i-1]
                end
                if BC.bot[2]==0 # vy imposed on bottom (Dirichlet)
                    dvydx=0.0
                elseif BC.bot[2]==3 # open bottom so set to be the same as adjacent node
                    dvydx=2.0*(vy[i,j-1]-vy[i,j-2])./(dx[j-1]+dx[j-2])
                elseif BC.bot[2]==4 # same
                    dvydx=2.0*(vy[i,j-1]-vy[i,j-2])./(dx[j-1]+dx[j-2])
                elseif BC.bot[2]==5 # same
                    dvydx=0.0
                else
                    println("Stopped in get_strain_rate:  EXY for corner elements not coded for BC.bot(2)=",
                    string(BC.bot[2]))
                    error("Program stopped")
                end
                EXY[i,j]=(dvxdy + dvydx)/2
            ...
            elseif i==Ny && j==1
                ...
                if BC.bot[2]==0 # vy imposed on bottom (Dirichlet)# rollers
                    dvydx=0.0
                elseif BC.bot(2)==3 # open bottom so set to be the same as adjacent node
                    dvydx=2.0*(vy[i,j+1]-vy[i,j])./(dx[j+1]+dx[j])
                end
                EXY[i,j]=(dvxdy + dvydx)/2
